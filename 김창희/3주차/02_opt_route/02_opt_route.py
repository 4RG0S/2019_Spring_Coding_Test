...
def main():
	with open('opt_route_input.txt', 'r') as data:
		testcase_count = int(data.readline().strip())

		for testcase_num in range(1, testcase_count + 1):
			cusomter_count = int(data.readline().strip())
			location_list = list(map(int, data.readline().strip().split(' ')))
